...
policy = st.sidebar.radio("Scheduling Policy", ["Proportional Fairness (approx.)", "Equal Time Allocation"])
st.sidebar.markdown("---")
...
def path_loss_wifi(distance_m, freq_ghz, env="Indoor", cond="LOS"):
    ...
    return 32.4 + 10 * n * math.log10(distance_m) + 20 * math.log10(freq_ghz)
...
for idx, point in enumerate(st.session_state.clicked_points):
    Marker([point['lat'], point['lng']],
           tooltip=f'CPE {idx+1}',
           icon=folium.Icon(color='green', icon='signal')).add_to(m)
steps = 10
half_bw = ap_bw / 2.0
meters_per_deg_lat = 111320.0
meters_per_deg_lon = 40075000.0 * math.cos(math.radians(ap_lat)) / 360.0
bw_hz = bw_mhz * 1e6
noise_floor = noise_floor_dbm(bw_hz, noise_figure_db, impl_loss_db=1.0)
snr_min_db = 5.0
rx_min_dbm = noise_floor + snr_min_db + obss_penalty_db
pl_allow_db = ap_tx_dbm + ap_gmax_dbi + ue_gain_dbi - rx_min_dbm - impl_loss_db - fading_loss_db
...
for i in range(steps):
    ...
    Polygon([(ap_lat, ap_lon)] + points + [(ap_lat, ap_lon)], color=color, fill=True, fill_opacity=0.15).add_to(m)

def _seed_cpe_on_sector_edge(idx_from_zero: int):
    radius = 0.9 * distance_m_max
    offset_deg = (-half_bw + (idx_from_zero % max(1, int(ap_bw))))
    bearing_deg = ap_az + offset_deg
...
if SF_AVAILABLE:
    result = st_folium(m, width=700, height=550, key="map")
    remaining = num_ues - len(st.session_state.clicked_points)
...
if st.session_state.clicked_points:
    ...
    # Use st.data_editor for full editability
...
# --- Throughput Calculation with Spectrum Noise ---
if st.session_state.clicked_points:
    ue_results = []
    phy_rates_dl = []
    phy_rates_ul = []
    ...
    bw_hz = bw_mhz * 1e6
    ap_max_phy_rate = MCS_TABLES[bw_mhz][-1] * num_streams
    ap_max_mac_rate = ap_max_phy_rate * efficiency
    ap_dl_capacity = ap_max_mac_rate * (tdd_ratio_dl / 100.0)
    ap_ul_capacity = ap_max_mac_rate * (tdd_ratio_ul / 100.0)
    for idx, point in enumerate(st.session_state.clicked_points):
        ue_lat = point['lat']
        ue_lon = point['lng']
        ...
        # --- Downlink (AP → CPE): SNR at CPE ---
        rx_dl_dbm = clamp_rssi(ap_tx_dbm + G_ap_dir + G_ue_dir - pl_db - impl_loss_db)
        noise_dl_dbm = max(ap_noise_dbm, cpe_noise_dbm)
        ...
        noise_ul_dbm = max(ap_noise_dbm, cpe_noise_dbm)
    ...
    # --- Throughput Capping at 70% of PHY Rate ---
    for i, ue in enumerate(ue_results):
        max_dl = 0.7 * phy_rates_dl[i]
        max_ul = 0.7 * phy_rates_ul[i]
        ue['DL Throughput (Mbps)'] = round(min(ap_dl_capacity / n_cpe, phy_rates_dl[i], max_dl), 2)
        ue['UL Throughput (Mbps)'] = round(min(ap_ul_capacity / n_cpe, phy_rates_ul[i], max_ul), 2)
    ...
    st.dataframe(df[[
        ...
        'rx_dl_dbm', 'snr_dl_db', 'MCS_DL', 'PHY Rate DL (Mbps)', 'DL Throughput (Mbps)',
        'rx_ul_dbm', 'snr_ul_db', 'MCS_UL', 'PHY Rate UL (Mbps)', 'UL Throughput (Mbps)',
